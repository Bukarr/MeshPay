...
export const TransactionsPage: React.FC<TransactionsPageProps> = ({
  ...
  const filtered = transactions.filter(t => {
    ...
    if (filterType === 'usd_to_ngn' && t.type !== 'usd_to_ngn') return false;
    if (filterType === 'nearby' && t.type !== 'nearby_send' && t.type !== 'nearby_receive') return false;
    if (filterType === 'top_up' && t.type !== 'top_up') return false;
    if (filterType === 'queued' && t.status !== 'queued_offline') return false;

    // Apply quick-filter direction toggle
    const isReceived = t.type === 'nearby_receive' || t.type === 'top_up';
  ...
  const getGroupTitle = (timestampStr: string) => {
    try {
  ...
  // Group filtered transactions preserving order
  const groupedList: { title: string; items: Transaction[] }[] = [];
  filtered.forEach(tx => {
    const title = getGroupTitle(tx.timestamp);
    let group = groupedList.find(g => g.title === title);
    if (!group) {
      group = { title, items: [] };
  ...
  return (
    <div className="space-y-4 pb-20 pt-3 px-4 max-w-md mx-auto text-slate-800 animate-fadeIn">
      {/* Header Card */}
      <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm space-y-3">
        ...
        {/* Direction Segmented Control Toggle */}
        <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200/50">
          {[
            ...
            <button
              key={view.id}
              onClick={() => setDirectionFilter(view.id as any)}
              className={`flex-1 py-1.5 rounded-xl text-xs font-bold transition-all ${
                directionFilter === view.id
        ...
        {filtered.length === 0 ? (
          ...
          groupedList.map((group) => (
            <div key={group.title} className="space-y-3 relative">
              {/* Sticky Date Header */}
              <div className="sticky top-14 bg-white/95 backdrop-blur-md py-1.5 px-2 z-10 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100/60 rounded-md">
                {group.title}
              </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-[11px] text-slate-500 font-mono mt-0.5">
                            {new Date(tx.timestamp).toLocaleString([], { hour: '2-digit', minute: '2-digit' })} • {tx.recipientDetail}
                          </div>
